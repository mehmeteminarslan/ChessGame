...
public class Pawn extends Piece{
	...
	public boolean canMove(int x, int y,int selectedSquareX,int selectedSquareY,Piece[][] pieces) {
		// TODO Auto-generated method stub
		if(isBlack){
			if((y == 1 && x == 0)||((y==2&&selectedSquareY==1)&&pieces[selectedSquareX][selectedSquareY+1]==null)){
				return true;
			}
			else{
				return false;
			...
			if((y == -1 && x == 0)||((y==-2&&selectedSquareY==6)&&pieces[selectedSquareX][selectedSquareY-1]==null)){
				return true;
			}
			else{
				return false;
